...
// Batch lookup of emails for many uids. Identity Toolkit accepts up to 100
// localIds per call. We do a single call here since we're well under that.
export async function getEmailsForUids(env, token, uids) {
	...
	if (!res.ok) {
		console.error(`Batch email lookup failed: ${res.status}`);
		return {};
	}
	const data = await res.json();
	const out = {};
	for (const u of data.users || []) {
		if (u.localId && u.email) out[u.localId] = u.email;
	}
	return out;
}

// ----- Consensus events (dedup state) -----

export async function getConsensusEvent(env, token, tokenMint) {
	const url = `${FIRESTORE_BASE}/projects/${env.FIREBASE_PROJECT_ID}/databases/(default)/documents/consensusEvents/${tokenMint}`;
	const res = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
	if (res.status === 404) return null;
	if (!res.ok) {
		console.error(`Get consensus event failed for ${tokenMint}: ${res.status}`);
		return null;
	}
	const data = await res.json();
	const f = data.fields || {};
	return {
		tokenMint,
		lastFiredAt: f.lastFiredAt?.timestampValue || null,
		walletCount: Number(f.walletCount?.integerValue || 0),
	};
}

export async function saveConsensusEvent(env, token, tokenMint, { walletCount, wallets }) {
	const url = `${FIRESTORE_BASE}/projects/${env.FIREBASE_PROJECT_ID}/databases/(default)/documents/consensusEvents/${tokenMint}`;
	const body = {
		fields: {
			tokenMint: { stringValue: tokenMint },
			lastFiredAt: { timestampValue: new Date().toISOString() },
			walletCount: { integerValue: String(walletCount) },
			wallets: {
				arrayValue: {
					values: (wallets || []).slice(0, 25).map((w) => ({
						mapValue: {
							fields: {
								wallet: { stringValue: w.wallet || "" },
								label: { stringValue: w.label || "" },
								signature: { stringValue: w.signature || "" },
								solAmount: { doubleValue: w.solAmount || 0 },
								ts: { integerValue: String(w.ts || 0) },
							},
						},
					})),
				},
	...
	if (!res.ok) {
		console.error(`Save consensus event failed for ${tokenMint}: ${res.status} ${await res.text()}`);
	}
}
